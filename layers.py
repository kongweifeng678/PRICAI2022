...
def compute_D(points, norm):
    ...
    points = points.float()
    D = - norm @ points  # b, H*W  @矩阵乘法

    return D    

# ----- vps
def depth2norm(cam_points, height, width, nei=3):  # cam_points [b,c,h*w]
    #pts_3d_map = cam_points[:, :3, :].permute(0,2,1).view(-1, height, width, 3)
    pts_3d_map = cam_points
    ...
    ## generate difference between the central pixel and one of 8 neighboring pixels  中间点对八个方向求L1
    ...
    normal_map = F.pad( normal_vector, (0,2*nei,2*nei,0),"constant",value=0)  # pad 左右上下
...
def compute_mmap(batch_size, norm, vps, H, W, epoch, nei):
    ...
    score, index = torch.max(cos_sim, 1)   # 返回索引
    mmap = index.reshape(batch_size, 1, H, W)  # reshape之后每个值保留的都是0~5个方向
    ...
    if torch.any(torch.isnan(score_map)):
        ...
        score_map = torch.where(torch.isnan(score_map), torch.full_like(score_map, 1),
                                   score_map)
...
def align_smooth_norm(batch_size, mmap, vps, H, W):
    ...
    smooth_norm = torch.gather(vps, 2, mmap_label)  # 根据index找到相应的align normal
