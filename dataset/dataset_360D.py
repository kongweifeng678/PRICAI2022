...
class Dataset360D(Dataset):
    #360D Dataset#
    def __init__(self, filenamesFile, delimiter, mode, inputShape, vps_path="/data/kongweifeng/GithubFile/360Depth/ss360/SVS_norm/vps/",transform=None, rescaled=False):
    #def __init__(self, filenamesFile, delimiter, mode, inputShape,transform=None, rescaled=False):
        ...
        #self.vps_path = "/data/kongweifeng/GithubFile/360Depth/ss360/SVS_norm/vps/"  #这里的路径还要确定
    ...
    def loadItemUD(self, idx):
        ...
        if (idx >= self.length):
            ...
            if self.rescaled:
                leftRGB2 = self.pilToTensor(self.resize2(leftRGB))  # size//2
                leftRGB4 = self.pilToTensor(self.resize4(leftRGB))  # size//4
            ...
            vp = self.vps_path + '%d.npy'%(idx+1)
            vps = np.load(vp)  # 不考虑数据增强的情况下是可行的
            vps = vps.reshape([3,3])
            vps = np.vstack([vps, -vps]).astype(np.float32)
            #
            # # SVO keypoint
            # svo_map_resized = np.zeros((self.height, self.width))  # 256*512
            # img = np.array(leftRGB)
            ...
            # input_svo_map = torch.from_numpy(svo_map_resized.copy())  # 传入字典
            # svo_map_resized[noise_points[:, 0], noise_points[:, 1]] = 1
            #
            # keypoints = np.concatenate((key_points, noise_points), axis=0)
            # input_dso_points = torch.from_numpy(keypoints).float()
