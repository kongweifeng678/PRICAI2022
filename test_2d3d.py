...
def parse_arguments(args):
    ...
    # enumerables
    parser.add_argument('-b',"--batch_size", type=int, default=1, help="Test a <batch_size> number of samples each iteration.")    
    ...
    parser.add_argument("--vis_path", default="/data/kongweifeng/GithubFile/360Depth/ss360/SVS_norm_planar/vis_img/shanghai/kong", type=str, help="path to the dataset.")      
    return parser.parse_known_args(args)

def compute_errors(gt, pred, invalid_mask, weights, sampling, mode='cpu', median_scale=False):
    b, _, __, ___ = gt.size()
    scale = torch.median(gt.reshape(b, -1), dim=1)[0] / torch.median(pred.reshape(b, -1), dim=1)[0]# if median_scale else torch.tensor(1.0).expand(b, 1, 1, 1).to(gt.device) 
...
if __name__ == "__main__":
    ...
    #sf2d3d
    #test_data = dataset.stanford2d3d('/data/dataset/Standford2D3D/', '/data/dataset/Standford2D3D/stanford2d3d_test.txt',256,512)
    #shanghai360
    test_data = dataset.stanford2d3d.Stanford2D3D('/data/dataset/shanghai360', '/data/dataset/shanghai360/final_test.txt',256,512)
    ...
    #sf2d3d
    #errors = numpy.zeros((7, 373), numpy.float32)    
    #shanghai360
    errors = numpy.zeros((7, 275), numpy.float32) 
    ...
    with torch.no_grad():            
        ...
        for test_batch_id , test_batch in enumerate(test_data_iterator):
            ''' Data '''
            left_rgb = test_batch['rgb'].to(device)            
            left_depth = test_batch['leftDepth'].to(device)
            name = '1'
            # if 'righttRGB' in test_batch:
            ...
            #gt_scale = left_depth[0].squeeze().cpu().numpy()
            #pred_scale = left_depth_pred[0].squeeze().cpu().numpy()
            #ratio = numpy.median(gt_scale) / numpy.median(pred_scale)
            #ratio = numpy.array(ratio)
            #left_depth_pred *= torch.from_numpy(ratio).to(device)
            ''' Errors '''
            ...
            if args.save_samples:
                ...
                saver.save_samples(test_batch['rgb'], left_depth, left_depth_pred)
            ''' Visualize & Append Errors '''
            for i in range(b):
                #idx = counter + i
                idx = test_batch_id * args.batch_size + i
